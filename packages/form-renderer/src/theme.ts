...
export function getThemeStyle(theme: FormTheme, query?: Record<string, any>): string {
  ...
  .heyform-theme-background {
    ...
    ${theme.backgroundImage ? (helper.isURL(theme.backgroundImage) || theme.backgroundImage.startsWith('/') ? `background-image: url(${theme.backgroundImage});` : `background-image: ${theme.backgroundImage}`) : ''}
  }
